...
async function enviarFormulario(datosUsuario){
  try {
    ...
    if (response.status === 201) {
      ...
      habilitarBotones();
    } else if (resultado ["Code"]== "ENVIO-MAIL-ERROR") {
      alert("Ocurrió un error inesperado y su usuario no fue registrado, intentelo nuevamente.");//     + (resultado.DatabaseError || resultado.Error));
      habilitarBotones();
